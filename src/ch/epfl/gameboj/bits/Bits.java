...
public final class Bits {
    ...
    private static int[] t = new int[] { 0x00, 0x80, 0x40, 0xC0, 0x20, 0xA0, 0x60, 0xE0,
    ...
    /**
     * Returns a mask with a single 1 at the given index.
     *
     * @param index integer
    ...
    /**
     * Returns true if the bit in the given bits is 1.
     *
     * @param bits the integer which "index" bit is tested
     * @param bit  the bit we want to test
     * @return true if the bit at the given index is 1, false if it is 0
     */
    ...
    /**
     * Returns the same value as bits but with a 1 (if newValue = true) or a 0
     * (if newValue = false) at the given index.
     *
     * @param bits     the integer which value is returned with one bit changed
    ...
    /**
     * Returns a value which 'size' LSB equal the ones of bits from 'start'
     * (included) to 'start + size' (excluded).
     *
     * @param bits  the integer which bits are extracted
    ...
    /**
     * Returns a value with the 'size' LSB of bits to which a rotation of the
     * given distance has been applied.
     *
     * @param size     the number of bits to rotate
    ...
    public static int rotate(int size, int bits, int distance) {
        ...
        int leftBits = bits << d;
        int rightBits = bits >>> (size - d);

        return clip(size, leftBits | rightBits);
    }

    /**
     * Returns b with the value of b's 7th bit extended to the 31st bit.
     *
    ...
    /**
     * Returns the 8 LSB of b which were reversed.
     *
    ...
    /**
     * Returns the 8 LSB of b which were conversed (One's complement).
     *
    ...
    /**
     * Returns an integer which MSB are highB's bits and which LSB are lowB's
     * bits.
     *
     * @param highB the MSB of the returned integer
